<!DOCTYPE html>
	...
	function init() {
        ...
        <?php 
            include("bd.php");
            $query = mysqli_query($link, "SELECT * FROM cams_min_distance_center_2");
            while ($result = mysqli_fetch_assoc($query)){
                $latitude = $result["latitude"];
                $longitude = $result["longitude"];
                if ($result["id_center"] == 5702){
        ...
        <?php 
                ...
                else if($result["id_center"] == 9129){ ?>
                 var long = <?php echo $longitude ?>;
        var lat = <?php echo $latitude ?>;
                    myPlacemark = new ymaps.Placemark([lat, long], {}, {  // Координаты метки объекта
                        preset: "islands#violetDotIcon" // Тип метки
                    });
                    myMap.geoObjects.add(myPlacemark); <?php 
                }?>
                <?php 
            
            if($result["id_center"] == 11351){ ?>
             var long = <?php echo $longitude ?>;
        var lat = <?php echo $latitude ?>;
                myPlacemark = new ymaps.Placemark([lat, long], {}, {  // Координаты метки объекта
...
from
	Open_date_sound
where
	latitude between 55.460531 - 0.009009 and 55.460531 + 0.009009
	and longitude between 37.210488 - 0.009009 and 37.210488 +  0.009009;